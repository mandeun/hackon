...
회원가입이 없습니다. 대회를 만들면 열쇠가 하나 나오고, 그 열쇠를 가진 사람이 운영자입니다.
로그인(카카오·구글·네이버)은 여러 기기에서 같은 대회를 열기 위한 선택일 뿐입니다.
...
- **권한은 열쇠로 나눕니다.** 운영·주최자·심사·투표·팀·받는 사람·후원 열쇠가 있고 각각 여는
  범위가 다릅니다. 표와 주소와 열쇠는 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md) 에 적었습니다.
- **로그인은 선택입니다.** 카카오·구글·네이버 중 켜 둔 것으로 들어오면 열쇠를 안 들고 다녀도
  여러 기기에서 내 대회가 열립니다. 키를 안 주면 그 주소는 아예 없습니다. 켜는 법과
  «다른 로그인으로 온 같은 사람» 을 알아보는 규칙은 [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md#로그인) 에 있습니다.
